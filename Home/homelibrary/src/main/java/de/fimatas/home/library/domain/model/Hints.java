    ...
    public List<String> formatAsText(boolean onlySolid, boolean withRoomName, RoomClimate roomClimate) {
        ...
        for (Hint hint : getHintMap().keySet()) {
            if (getHintMap().get(hint) == HintState.OFF) {
                ...
                && (getHintMap().get(hint) != HintState.ACTIVE_HYSTERESIS && getHintMap().get(hint) != HintState.ACTIVE)) {
                // skip
            } else if (!onlySolid
                && (getHintMap().get(hint) != HintState.OFF_HYSTERESIS && getHintMap().get(hint) != HintState.ACTIVE)) {
                // skip
            } else {
                if (withRoomName) {
